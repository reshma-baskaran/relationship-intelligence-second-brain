...
def source_id(source_url: str, content: str) -> tuple[str, str]:
    digest = hashlib.sha256((source_url.strip() + "\n" + content).encode("utf-8")).hexdigest()
...
def _existing_sources(register: Path) -> tuple[set[str], set[str]]:
    ...
    urls = {row.get("Source URL", "").strip() for row in rows}
    return ids, urls


def build_source_page(*, title: str, source_type: str, evidence_context: str, relationship_status: str, privacy: str, attribution: str, source_date: str, organization: str, source_url: str, source_identifier: str, content_hash: str, content: str) -> str:
    return "\n".join([
        "---",
        "type: source",
...
def ingest(*, vault: Path, source_file: Path, title: str, summary: str, source_url: str, source_date: str, source_type: str, evidence_context: str, relationship_status: str, privacy: str, attribution: str, organization: str, dry_run: bool = False) -> dict:
    ...
    identifier, digest = source_id(source_url, content)
    register = vault / "00_System/source-register.md"
    known_ids, known_urls = _existing_sources(register)
    if identifier in known_ids or source_url in known_urls:
    ...
    row = (
        ...
        f"{relationship_status} | {privacy} | {organization} | {attribution} | unprocessed | {source_url} | {safe_summary} | [[02_Raw Sources/{destination.stem}]] |"
    )
    result = {
